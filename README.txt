...
подготовить выгрузку с историей ГДИС на фонде из NGT Smart.
...
2) Заполнить необходимые параметры расчета в конфигурационном файле parameters.yml в папке /conf_files.
	...
	--- data_file содержит название к исходного файла с данными
		...
		-- Лист с названием "ГДИС" содержит выгрузку данных по ГДИС формата NGT Smart, либо остается пустым, если
		   ГДИС не нужно учитывать
		...
		-- Лист с названием "Проектный фонд" содержит таблицу с данными по проектным скважинам со следующими
		   ...
		   Если проектных скважин нет, лист необходмо оставить пустым
	...
	*ННС сразу же попадают в зону охвата, так как являются точками при расчете.
	...
	от среднего дебита по объекту
		...
		Варианты значений параметра: "да" - не учитывать добывающие скважины с дебитом выше указанного в
					     ...
					     "нет" - учитывать добывающие скважины с дебитом выше указанного в
					            в параметре percent_oilrate процента от среднего дебита по объекту
